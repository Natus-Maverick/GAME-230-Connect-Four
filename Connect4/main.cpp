...
bool check(int x, int y, char s);	//check winner
...
int main()		//main function
{
	...
	while (check(x, y, 'O') != true)		//Player 1 goes with "X", check win condition for Player 2 before player 1's turn.
	{
		board();
		int top = 0;
		...
		if (check(x, y, 'X') != true)		//Check win condition for Player 1 before Player 2's turn.
		{
			board();
...
bool check(int x, int y, char s)		//Check if "X" or "O" (declared by char s) win the game.
{
	int horizonal = 0;
	int vertical = 0;
	int diagonalL = 0;
